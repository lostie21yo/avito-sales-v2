...
def corsel_check(df, donor_link, discount, headers, yandex_image_folder_path, annex, check_new, excel_file_name, currencies):
    

    domain = "https://www.corsel.ru"

    katalog_html = BS(requests.get(donor_link).content, 'html.parser')
    categories = katalog_html.find_all("div", {"class": "item_block"})
    category_number = 0
    for top_category_div in categories:
        category_number += 1
        new_count = 0
        category_name = top_category_div.find("li", {"class": "name"}).a.text.strip()
        if category_name != "Компрессоры воздушные":
            continue

        print(f'Категория ({category_number}/{len(categories)}): {category_name}')
        category_link = f'https://www.corsel.ru{top_category_div.find("li", {"class": "name"}).a['href']}'
        top_category_html = BS(requests.get(category_link).content, 'html.parser')
        page_number = int(top_category_html.find("div", {"class": "nums"}).find_all('a')[-1].text)
        for p in trange(page_number):
            page_html = BS(requests.get(f"{category_link}/?PAGEN_1={p+1}").content, 'html.parser')
                ...
                # добавление новых позиций
                    ...
                    if check_new and (vendorCode not in df["Id"].values):
                        ...
                        try:
                            ...
                            imageUrls = "no data"
                            print(vendorCode, "image error:",  e)
